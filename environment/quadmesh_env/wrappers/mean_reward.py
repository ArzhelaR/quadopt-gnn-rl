...
class MeanRewardWrapper(gym.Wrapper):
    def __init__(self, env):
        super().__init__(env)
        self.mesh_mean_score= self.get_mean_mesh_score()

    def get_mean_mesh_score(self):
        """
        ...
        mesh_score = mesh_score / n_nodes
        return mesh_score

    def step(self, action):
        previous_mean_mesh_score = self.mesh_mean_score
        obs, _, terminated, truncated, info = self.env.step(action)
        if info["valid_action"]:
            self.mesh_mean_score = self.get_mean_mesh_score()
            reward = (previous_mean_mesh_score - self.mesh_mean_score)*10
        else:
            reward = -5
