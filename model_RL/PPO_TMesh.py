...
class Actor(nn.Module):
    def __init__(self, env, input_dim, n_actions, n_darts_observed, lr=0.0001, eps=0):
        ...
        self.gamma = 0.9
        self.optimizer = Adam(self.parameters(), lr=lr, weight_decay=0.001, betas=(0.99, 0.99), eps=1e-8)
        self.env = env
        self.eps = eps
    ...
    def reset(self, env=None):
        ...
        self.optimizer = Adam(self.parameters(), betas=self.optimizer.defaults['betas'], lr=self.optimizer.defaults['lr'], weight_decay=self.optimizer.defaults['weight_decay'], eps=self.optimizer.defaults['eps'])

    def select_action(self, observation, info, mask=None):
        ...
        if np.random.rand() < self.eps:
            ...
            # obs = torch.tensor(observation.flatten(), dtype=torch.float32) # used with restrain observation
            obs = torch.tensor(observation, dtype=torch.float32) # used with restrain observation
            values = self.forward(obs, mask)
            mask = torch.tensor(mask, dtype=torch.float32)
            masked_values = values.flatten() + (mask.flatten() - 1) * 1e5
            pmf = self.softmax(masked_values)
    ...
    def train(self, dataset):
        ...
        for _ in range(self.nb_epochs):
            ...
            while start < num_samples - 2:
                ...
                actor_loss = []
                for _, (ma, o, a, r, G, old_prob, next_o, done, st, ideal_s, mask, next_mask) in enumerate(batch, 1):
                    ...
                    values = self.actor.forward(o)
                    masked_values = values.flatten() + (mask.flatten() - 1) * 1e5
                    pmf = self.actor.softmax(masked_values)
            ...
            for iteration in tqdm(range(self.nb_iterations)):
                ...
                for _ in tqdm(range(self.nb_episodes_per_iteration)):
                    ...
                    while step < self.max_steps:
                        ...
                        obs = next_obs
                        mask = info["mask"]
                        action, prob = self.actor.select_action(obs, info, mask=mask)
                        if action is None:
                            wins.append(0)
                            break
                        ...
                        actions_info["nb_fuse"] += info["fuse"]
                        actions_info["nb_cleanup_bdy"] += info["cleanup_bdy"]
                        ...
                        actions_info["nb_invalid_fuse"] += info["invalid_fuse"]
                        actions_info["nb_invalid_cleanup_bdy"] += info["invalid_cleanup_bdy"]
                        ...
                        if terminated:
                            if truncated:
                                ...
                                trajectory.append((ma, obs, action, reward, prob, next_obs, done, st, s_ideal))
                            break
                        trajectory.append((ma, obs, action, reward, prob, next_obs, done, st, s_ideal, mask, next_mask))
                        step += 1
                    if len(trajectory) != 0:
                        #Compute G
                        G = 0
                        computed_trajectory = []
                        for ma, obs, action, reward, prob, next_obs, done, st, s_ideal, mask, next_mask in reversed(trajectory):
                            G = reward+0.9*G*(1-done)
                            computed_trajectory.append((ma, obs, action, reward, G, prob, next_obs, done, st, s_ideal, mask, next_mask))
                    ...
                    writer.add_scalar("actions/nb_fuse", actions_info["nb_fuse"], nb_episodes)
                    writer.add_scalar("actions/nb_cleanup_bdy", actions_info["nb_cleanup_bdy"], nb_episodes)
                    ...
                    if actions_info["nb_fuse"]> 0:
                        writer.add_scalar("actions/invalid_fuse",
                                          actions_info["nb_invalid_fuse"] * 100 / actions_info["nb_fuse"],
                                          nb_episodes)
                    else:
                        writer.add_scalar("actions/invalid_fuse", 0, nb_episodes)
                    if actions_info["nb_cleanup_bdy"] > 0:
                        writer.add_scalar("actions/invalid_cleanup_bdy", actions_info["nb_cleanup_bdy"] * 100 / actions_info["nb_cleanup_bdy"], nb_episodes)
                    else:
                        writer.add_scalar("actions/invalid_cleanup_bdy", 0, nb_episodes)
                self.train(dataset)
